function [train_result_matrix, DistSample_frst_norm_b,DistSample_frst_norm_m] =finddist_splitsets_1(train_result_matrix,Train_Set,QQ_trans_opt_b,QQ_trans_opt_m,min_val_b_opt,max_val_b_opt,min_val_m_opt,max_val_m_opt,threshold_dist_b,threshold_dist_m)
...
for count = 1: size(Train_Set,2)
    ...
    if abs(DistSample_frst_norm_b(count)- threshold_dist_b) <= abs(DistSample_frst_norm_m(count) - threshold_dist_m)
        
        train_result_matrix(count,b_col)=2;
    else
        train_result_matrix(count,m_col)=4;
    end
end
% train_result_matrix;
%DistSample_frst_norm_b
